...
/**
 * Landing Page P0 Critical Sections (Navbar menu, How It Works, Final CTA, Footer)
 * Adds fully native Elementor widgets with Editor=Front parity.
 * Markers: _element_id => pdfm_how_it_works, pdfm_final_cta, pdfm_footer
 */
function pdfm_run_landing_p0_critical(): void {
    $post_id = (int) get_option('page_on_front');
    if ($post_id <= 0) { $post_id = 11; }
    ...
    $already_all = true; foreach ($needles as $n) { if (strpos((string)$json, $n) === false) { $already_all = false; break; } }
    if ($already_all && ! get_option('pdfm_force_migrate')) { return; }
    ...
    $section_has_tools = function (array $section): bool {
        if (($section['elType'] ?? '') !== 'section') return false;
        $q = [$section]; $cnt = 0;
        while ($q) {
            $n = array_shift($q);
            if (($n['elType'] ?? '') === 'widget' && ($n['widgetType'] ?? '') === 'icon-box') {
                $t = strtolower((string)($n['settings']['title_text'] ?? '')); if (preg_match('/compress|merge|split|convert|pdf/', $t)) { $cnt++; }
            }
            if (!empty($n['elements'])) { foreach ($n['elements'] as $c) { $q[] = $c; } }
        }
        return $cnt >= 2;
    };

    $find_nav_text_widget = function (&$nodes) use (&$find_nav_text_widget, $node_text_contains) {
        if (!is_array($nodes)) return null;
        foreach ($nodes as $idx => &$n) {
            if (($n['elType'] ?? '') === 'widget' && ($n['widgetType'] ?? '') === 'text-editor') {
                $blob = strtolower(wp_json_encode($n['settings'] ?? []));
                if (strpos($blob, '<a') !== false && (strpos($blob, '#pricing') !== false || strpos($blob, 'pricing') !== false) && (strpos($blob, '#tools') !== false || strpos($blob, 'all tools') !== false)) {
                    return [ &$nodes, $idx ];
                }
            }
            if (!empty($n['elements'])) { $res = $find_nav_text_widget($n['elements']); if ($res) return $res; }
        }
        return null;
    };

    $make_icon_list_nav = function (): array {
        $items = [
            [ '_id' => wp_generate_password(6, false, false), 'text' => 'Pricing', 'link' => [ 'url' => '#pricing' ] ],
            [ '_id' => wp_generate_password(6, false, false), 'text' => 'All Tools', 'link' => [ 'url' => '#tools' ] ],
            [ '_id' => wp_generate_password(6, false, false), 'text' => 'FAQ', 'link' => [ 'url' => '#faq' ] ],
        ];
        return [
            'id' => 'w_' . wp_generate_password(8, false, false), 'elType' => 'widget', 'isInner' => false,
            'widgetType' => 'icon-list', 'settings' => [ 'icon_list' => $items, 'view' => 'inline', 'align' => 'right' ], 'elements' => []
        ];
    };

    // Replace ALL inline HTML menus with native icon-list nav
    $replaced_nav = false;
    $replace_nav = function (&$nodes) use (&$replace_nav, $make_icon_list_nav, &$replaced_nav) {
        if (!is_array($nodes)) return;
        foreach ($nodes as $i => &$n) {
            if (($n['elType'] ?? '') === 'widget' && ($n['widgetType'] ?? '') === 'text-editor') {
                $blob = strtolower(wp_json_encode($n['settings'] ?? []));
                if (strpos($blob, '<a') !== false && (strpos($blob, '#pricing') !== false || strpos($blob, 'pricing') !== false) && (strpos($blob, '#tools') !== false || strpos($blob, 'all tools') !== false)) {
                    $nodes[$i] = $make_icon_list_nav();
                    $replaced_nav = true;
                    continue;
                }
            }
            if (!empty($n['elements'])) { $replace_nav($n['elements']); }
        }
    };
    $replace_nav($data);

    // Find Tools section index
    $tools_index = -1;
    foreach ($data as $i => $sec) {
        if (($sec['elType'] ?? '') === 'section' && $section_has_tools($sec)) { $tools_index = $i; break; }
    }

    // How It Works section
    $make_badge = function (string $num): array {
        return [ 'id' => 'w_' . wp_generate_password(8, false, false), 'elType' => 'widget', 'isInner' => true, 'widgetType' => 'button', 'settings' => [ 'text' => $num, 'size' => 'sm', 'button_type' => 'primary', 'align' => 'center', 'border_radius' => [ 'unit' => 'px', 'top' => 999, 'right' => 999, 'bottom' => 999, 'left' => 999 ] ], 'elements' => [] ];
    };
    $make_step_col = function (string $num, string $title, string $desc) use ($make_badge): array {
        return [ 'id' => 'col_' . wp_generate_password(8, false, false), 'elType' => 'column', 'isInner' => true, 'settings' => [ '_column_size' => 33.33, 'vertical_align' => 'top' ], 'elements' => [
            $make_badge($num),
            [ 'id' => 'w_' . wp_generate_password(8, false, false), 'elType' => 'widget', 'isInner' => true, 'widgetType' => 'heading', 'settings' => [ 'title' => $title, 'header_size' => 'h3', 'align' => 'center' ], 'elements' => [] ],
            [ 'id' => 'w_' . wp_generate_password(8, false, false), 'elType' => 'widget', 'isInner' => true, 'widgetType' => 'text-editor', 'settings' => [ 'editor' => $desc, 'align' => 'center' ], 'elements' => [] ],
        ] ];
    };
    $how_section = [
        'id' => 'sec_' . wp_generate_password(8, false, false), 'elType' => 'section', 'isInner' => false,
        '_element_id' => 'pdfm_how_it_works',
        'settings' => [ 'layout' => 'boxed', 'content_width' => 'boxed', 'padding' => [ 'unit' => 'px', 'top' => 48, 'right' => 0, 'bottom' => 48, 'left' => 0 ], 'pdfm_marker' => 'how_it_works' ],
        'elements' => [
            [ 'id' => 'col_' . wp_generate_password(8, false, false), 'elType' => 'column', 'isInner' => false, 'settings' => [ '_column_size' => 100 ], 'elements' => [
                [ 'id' => 'w_' . wp_generate_password(8, false, false), 'elType' => 'widget', 'isInner' => false, 'widgetType' => 'heading', 'settings' => [ 'title' => 'Three Simple Steps', 'header_size' => 'h2', 'align' => 'center' ], 'elements' => [] ],
                [ 'id' => 'sec_' . wp_generate_password(8, false, false), 'elType' => 'section', 'isInner' => true, 'settings' => [ 'layout' => 'columns' ], 'elements' => [
                    $make_step_col('1', 'Upload Your File', 'Drag & drop your PDF, Word doc, or image. Up to 50MB per file.'),
                    $make_step_col('2', 'Pay $0.99', 'One-time payment via Stripe. No subscription, no auto-renewal, no surprises.'),
                    $make_step_col('3', 'Download Instantly', "Get your processed file immediately. That's it—you're done!"),
                ] ],
            ] ],
        ],
    ];
    ...
    if ($tools_index >= 0) {
        array_splice($data, $tools_index + 1, 0, [$how_section]);
    } else {
        $data[] = $how_section;
    }
    ...
    $security_items = [
        [ '_id' => wp_generate_password(6, false, false), 'text' => 'Money-Back Guarantee', 'selected_icon' => [ 'value' => 'fas fa-lock', 'library' => 'fa-solid' ] ],
        [ '_id' => wp_generate_password(6, false, false), 'text' => 'No Auto-Renewal', 'selected_icon' => [ 'value' => 'fas fa-lock', 'library' => 'fa-solid' ] ],
        [ '_id' => wp_generate_password(6, false, false), 'text' => 'No Subscription Ever', 'selected_icon' => [ 'value' => 'fas fa-lock', 'library' => 'fa-solid' ] ],
    ];
    $final_cta = [
        'id' => 'sec_' . wp_generate_password(8, false, false), 'elType' => 'section', 'isInner' => false,
        '_element_id' => 'pdfm_final_cta',
        'settings' => [
            'layout' => 'boxed', 'content_width' => 'boxed', 'padding' => [ 'unit' => 'px', 'top' => 64, 'right' => 0, 'bottom' => 64, 'left' => 0 ], 'pdfm_marker' => 'final_cta',
            'background_background' => 'gradient',
            'background_color' => '#2563EB',
            'background_color_b' => '#1E3A8A',
            'background_gradient_type' => 'linear',
        ],
        'elements' => [
            [ 'id' => 'col_' . wp_generate_password(8, false, false), 'elType' => 'column', 'isInner' => false, 'settings' => [ '_column_size' => 100, 'horizontal_align' => 'center' ], 'elements' => [
                [ 'id' => 'w_' . wp_generate_password(8, false, false), 'elType' => 'widget', 'isInner' => false, 'widgetType' => 'heading', 'settings' => [ 'title' => 'Ready to Stop Wasting Money on Subscriptions?', 'header_size' => 'h2', 'align' => 'center' ], 'elements' => [] ],
                [ 'id' => 'w_' . wp_generate_password(8, false, false), 'elType' => 'widget', 'isInner' => false, 'widgetType' => 'text-editor', 'settings' => [ 'editor' => 'Join 2M+ smart users who pay only for what they use. Process your first PDF in under 60 seconds.', 'align' => 'center' ], 'elements' => [] ],
                [ 'id' => 'w_' . wp_generate_password(8, false, false), 'elType' => 'widget', 'isInner' => false, 'widgetType' => 'button', 'settings' => [ 'text' => 'Try Any Tool — $0.99', 'size' => 'lg', 'button_type' => 'info', 'align' => 'center' ], 'elements' => [] ],
                [ 'id' => 'w_' . wp_generate_password(8, false, false), 'elType' => 'widget', 'isInner' => false, 'widgetType' => 'icon-list', 'settings' => [ 'icon_list' => $security_items, 'view' => 'inline', 'align' => 'center' ], 'elements' => [] ],
            ] ],
        ],
    ];

    // Footer (4 columns + bottom row)
    $make_menu = function (string $heading, array $items): array {
        $list = [];
        foreach ($items as $t => $url) { $list[] = [ '_id' => wp_generate_password(6, false, false), 'text' => $t, 'link' => [ 'url' => $url ] ]; }
        return [
            'id' => 'col_' . wp_generate_password(8, false, false), 'elType' => 'column', 'isInner' => true, 'settings' => [ '_column_size' => 25 ], 'elements' => [
                [ 'id' => 'w_' . wp_generate_password(8, false, false), 'elType' => 'widget', 'isInner' => true, 'widgetType' => 'heading', 'settings' => [ 'title' => $heading, 'header_size' => 'h5' ], 'elements' => [] ],
                [ 'id' => 'w_' . wp_generate_password(8, false, false), 'elType' => 'widget', 'isInner' => true, 'widgetType' => 'icon-list', 'settings' => [ 'icon_list' => $list ], 'elements' => [] ],
            ],
        ];
    };
    $menu_tools   = $make_menu('Tools', [ 'Compress PDF' => '#tools', 'Merge PDFs' => '#tools', 'Split PDF' => '#tools', 'Convert to PDF' => '#tools' ]);
    $menu_company = $make_menu('Company', [ 'About Us' => '#', 'Privacy Policy' => '#', 'Terms of Service' => '#', 'Contact' => '#' ]);
    $menu_support = $make_menu('Support', [ 'Help Center' => '#', 'Refund Policy' => '#', 'Volume Pricing' => '#' ]);

    $footer = [
        'id' => 'sec_' . wp_generate_password(8, false, false), 'elType' => 'section', 'isInner' => false,
        '_element_id' => 'pdfm_footer',
        'settings' => [ 'layout' => 'boxed', 'content_width' => 'boxed', 'background_background' => 'classic', 'background_color' => '#1F2937', 'padding' => [ 'unit' => 'px', 'top' => 48, 'right' => 0, 'bottom' => 24, 'left' => 0 ], 'pdfm_marker' => 'footer' ],
        'elements' => []
    ];

    // Place final CTA before footer
    $data[] = $final_cta;
    $data[] = $footer;

    // Save
    if (method_exists($document, 'set_elements_data')) { $document->set_elements_data($data); $document->save([]); }
    update_post_meta($post_id, '_elementor_data', wp_slash(wp_json_encode($data)));
    if (class_exists('Elementor\\Plugin')) { \Elementor\Plugin::$instance->files_manager->clear_cache(); }
    wp_cache_flush();
    if (get_option('pdfm_force_migrate')) { delete_option('pdfm_force_migrate'); }
}
